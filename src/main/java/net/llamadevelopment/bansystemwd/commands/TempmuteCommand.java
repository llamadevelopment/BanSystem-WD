...
public class TempmuteCommand extends Command {
    ...
    public TempmuteCommand(Config config, BanSystemWD instance) {
        super(config.getString("Commands.TempmuteCommand.Name"), config.getString("Commands.TempmuteCommand.Permission"), config.getStringList("Commands.TempmuteCommand.Aliases").toArray(new String[]{}));
                ...
                if (args[1].equalsIgnoreCase("days") || args[1].equalsIgnoreCase("hours")) {
                    ...
                    try {
                        ...
                        this.instance.getProvider().playerIsMuted(player, isMuted -> {
                            ...
                            this.instance.getProvider().mutePlayer(player, finalReason1, sender.getName(), finalSeconds1);
                            sender.sendMessage(Language.get("player.muted", player));
                        });
                    } catch (NumberFormatException exception) {
                        sender.sendMessage(Language.get("invalid.number"));
                    }
                } else sender.sendMessage(Language.get("usage.tempmutecommand", this.getName()));
            } else sender.sendMessage(Language.get("usage.tempmutecommand", this.getName()));
